...
async function triggerUpdateListings(owner, repo, pat, listings){
  const url=`https://api.github.com/repos/${owner}/${repo}/dispatches`;
  const payload={event_type:"update-listings",client_payload:{listings}};
...
function savePAT(){
  ...
  if(t) localStorage.setItem('crest_pat',t);
}

async function dispatchUpdate(){
  const pat=localStorage.getItem('crest_pat');
  const owner="Falcon23956";
  const repo="Crestvista-website";
  const listingsData = await fetch('listings.json').then(r=>r.json());
  const ok=await triggerUpdateListings(owner,repo,pat,listingsData.listings);
  alert(ok?"Dispatched":"Failed");
}
